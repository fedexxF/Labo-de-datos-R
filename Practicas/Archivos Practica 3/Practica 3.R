  ...
  for (j in 1:14) {
    for (k in 1:4) {
      for (t in 1:12) {  #Uso 12 y no 72 (ya que sino da error). Uso 12 porque son 12 meses
        prom_mensual[i,j,k,t]<-mean(temp_mensual[i,j,k,t,])     #Dejo la ultima dim vacia porque son años y solo quiero completar con los meses osea con t que va desde 1 a 12
      }
...
temp_anual
media_anual<-apply(temp_anual,c(1,2,3,5),mean) #Sacame el promedio PERO de la dim 4 que son los años. La 4 son los meses que no queremos jaja
media_anual 
  ...
  for (j in 1:14) {
    for (k in 1:4) {
      for (t in 1:6) {  #Uso 6 porque el tiempo son 6 a?os
        prom_anual[i,j,k,t]<-mean(temp_anual[i,j,k,,t]) #Dejo la dim 4 (MESES en blanco) Solo necesito que me complete la variable t, en la dim 6, osea años
      }
