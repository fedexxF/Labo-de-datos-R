...
DEF<-cbind(tabla1[,1],tabla1[,2],tabla1[,12])
MAM<-cbind(tabla1[,3],tabla1[,4],tabla1[,5])
MAM
JJA<-cbind(tabla1[,6],tabla1[,7],tabla1[,8])
SON<-cbind(tabla1[,9],tabla1[,10],tabla1[,11])

DEF_PROM<-rowMeans(DEF,na.rm = TRUE)
MAM_PROM<-rowMeans(MAM,na.rm = TRUE)
MAM_PROM
JJA_PROM<-rowMeans(JJA,na.rm = TRUE)
SON_PROM<-rowMeans(SON,na.rm = TRUE)


Estacion_PROM<-cbind(DEF_PROM,MAM_PROM,JJA_PROM,SON_PROM)
Estacion_PROM



# Genero un array de 66x4 vacío donde guardar los datos
#Tabla_Completar <- array(NA, c(66,4))
#Tabla_Completar
#Le pongo nombre a las columnas
#colnames(Tabla_Completar) <- colnames(tabla1)
...
#for(i in 1:4){
#  Tabla_Completar[,i] <- Estacion_PROM[1:66,i]  #a tabla de 1 a 7 y de columnas i (1 a 12). Guardalas por columnas en Tabla_Completar
#}
#Tabla_Completar
