...
#Este codigo realiza la apertura de 8 archivos binarios, la extraccion de las variables necesarias para evaluar
#el criterio de Bonner a una presion de 850hPa(tomando como capa superior 650hPa).
#Genera un grafico de 4 paneles que muestran la evolucion cada 12 hs del criterio de Bonner y el viento en vecores
#Ademas,almacena la variable Criterio de Bonner en un archivo binario con su correspondiente archivo de control
...
#Para estimar la posiciÃÂ³n del LLJ en el norte argentino se puede utilizar el criterio de
#Bonner, donde la condiciÃÂ³n para que se cumpla el mismo es que el viento en el nivel
#elegido sea superior a 12 m/s y la cortante vertical del viento entre ese nivel y un nivel
#superior supere los 6 m/s. Se tienen los datos de anÃÂ¡lisis del GDAS, de un evento de
...
#d) Graficar en 4 paneles la evoluciÃÂ³n del criterio de Bonner junto con el viento en
#vectores.
#e) Guardar la variable del criterio de Bonner en un archivo binario de doble precisiÃÂ³n y
#Little Endian. 
#(Para los puntos donde no se cumple el criterio definir un valor de undef). Crear un archivo de control (header).
...
#Cargamos un mapa del mundo

mapa <-map_data("world") 

#Podemos usar geom_path() o geom_polygon(), este ultimo colorea los contientes
# Usamos"group" para que se unan correctamente los puntos.
...
#Le agrego un poco de estilo

mi_mapa <- geom_path(data=mapa,                       
                     aes(x=long,y=lat,group=group),
                     size=.5,                         #Grosor de las lineas
...
#El de los archivos va de 0 a 360

#Usamos la funciÃÂ³n de metR para convertir longitudes:

df$lon <- ConvertLongitude(df$lon) 

#Preparando los datos del Criterio de Bonner para graficar -----------------------

#Tomamos los datos del Criterio de Bonner para los tiempos 2,4,6,8 (cada 12 hs)
...
#Graficamos y guardamos en una varbiale el grafico del criterio de Bonner + viento en 850hPa --------
...
#Le indicamos donde tiene que fijarse para separar los datos(en que columnas del data frame)
#y como queremos separar la zona de plot, en 2 columnas en este caso
...
#Leemos el archivo
readBin(paste0(salidas,"CriterioBonner.bin"),"numeric",n=length(Criterio_de_Bonner_Evaluacion),size = 4,endian = "little" )
